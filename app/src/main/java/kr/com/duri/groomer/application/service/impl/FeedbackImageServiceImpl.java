...
public class FeedbackImageServiceImpl implements FeedbackImageService {
    ...
    public void saveFeedbackImages(Feedback feedback, List<MultipartFile> images) {
        ...
        images.forEach(image -> {
            ...
            feedbackImageRepository.save(FeedbackImage.createNewFeedbackImage(feedback, imageUrl));
        });
    }
    ...
    public List<String> findFeedbackImagesByFeedback(Feedback feedback) {
        List<FeedbackImage> feedbackImages = feedbackImageRepository.findFeedbackImagesByFeedback(feedback);
        return feedbackImages != null ? feedbackImages.stream().map(FeedbackImage::getImageUrl).toList() : List.of();
    }
}
